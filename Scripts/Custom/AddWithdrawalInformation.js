        ...
        if (successResponseData.Success == true) {
            ...
            if (successResponseData.withdrawlslist.length > 0) {
                $.each(successResponseData.withdrawlslist, function (item, index) {
                    ...
                    html += '<h4 class="text-blue f_15 text-bold mb-0"  Title="' + index.Name + '">' + (index.Name.length > 10 ? (index.Name.substring(0, 8) + '..') : (index.Name)) + '</h4><span  class="f_12 margin-top-5 inlineBlock text-grey" Title="' + index.institutecode + '">' + (index.institutecode.length > 10 ? (index.institutecode.substring(0, 10) + '..') : (index.institutecode)) + '</span>'
                    html += '</div></td><td  class="noExl" Title="' + index.email + '">' + (index.email.length > 22 ? (index.email.substring(0, 20) + '..') : (index.email)) + '</td><td>' + index.ContactNumber + '</td><td>' + index.CreatedTime + '</td><td>' + index.WithDrawAmount + ' ' + index.Code + '</td>'
